...
<div className='w-full lg:w-[50%] h-full bg-white flex flex-col items-center p-2.5 gap-5'>
  ...
  <div className='flex gap-2.5 items-center text-[20px] font-semibold mt-10'>
    ...
    <img src={vImg} alt="" className='w-17.5'/>
  </div>
  ...
  <div className='relative flex items-center justify-start w-[90%] h-12.5 rounded-2xl mt-7.5 border-2 border-black' onClick={()=>setInputClicked({...inputClicked,name:true})}>
    <label htmlFor='name' className={`text-gray-700 absolute left-5 p-1.25 bg-white text-[15px] ${inputClicked.name?"-top-3.75":""}`}> Enter Your Name</label>
        <input type="text" id='name' className='w-full h-full rounded-2xl px-5 outline-none border-0' required onChange={(e)=>setName(e.target.value)} value={name}/>
...
<button className='w-[70%] px-5 py-2.5 bg-black text-white font-semibold h-12.5 cursor-pointer rounded-2xl mt-7.5' onClick={handleSubmit} disabled={loading}>{loading?<ClipLoader size={30} color='white'/>:"Sign Up"}</button>
<p className='cursor-pointer text-gray-800' onClick={()=>navigate("/signin")}>Already Have An Account ? <span className='border-b-2 border-b-black pb-0.75 text-black'>Sign In</span></p>
</div>
...
<img src={vImg} alt="" className='w-[40%]'/>
